...
- 스킬과 에이전트 지시가 열거나 실행하는 모든 플러그인 경로가 이제 플러그인 루트에 고정되므로, 작업 대상 프로젝트에 같은 이름의 파일이 있어도 그것이 플러그인 지시로 읽히거나 대신 실행될 수 없습니다.
- 경로 가드가 이제 백슬래시를 슬래시와 동일하게 인식하여, 같은 앵커 없는 참조가 `scripts/x.js`로 쓰면 잡히고 `scripts\x.js`로 쓰면 보이지 않던 우회를 막습니다. Windows가 지원 호스트이므로 문자 하나로 검사 전체가 무력화되던 문제입니다.
- 같은 가드가 반복된 구분자도 인식합니다. 백슬래시가 두 번 이어지면 이의는 제기되지만 그 지시가 실제로 심어둔 파일에 도달함을 증명하는 검사가 멈추던 좁은 경우를 막습니다.
- 유지보수자 룰북 경로를 플러그인이 배포하지 않는 경로로 선언하고 런타임에 절대 열지 말 것을 명시했습니다. 그 경로는 분석 대상 프로젝트에서만 resolve될 수 있기 때문입니다.

### Changed

- 스킬·에이전트 description이 약 45% 짧아졌으며 트리거 문구는 모두 그대로입니다.

### Fixed

- 감사는 7개가 아니라 6개의 sub-check를 수행하며, 읽기 전용 검사가 lock 획득 전에 실행되어 stale lock도 리포트에 남는다는 점을 명시합니다.
- 초기화는 쓰기 불가능한 memory root가 설정을 중단시킨다고 더 이상 말하지 않습니다. 중단시키는 것은 network root 거부뿐이며, 그 검사도 파일시스템 타입 조회가 아니라 경로 패턴 판정입니다.
- 승격은 lexical 인덱스만 갱신합니다. 기존에 명시하던 vector 인덱스 갱신은 실제로 일어나지 않았습니다.
- 삭제는 감사 로그에 요청을 기록할 뿐 카드 본문이나 인덱스 행을 제거하지 않으며, 스킬이 제거를 약속하는 대신 이 사실을 그대로 알립니다.
- 자율 도구 표면을 통한 내보내기는 교차 프로젝트뿐 아니라 모든 scope에서 거부됩니다.
- 삭제·수집 상한·수집 요약 파일이 실제 동작대로 서술됩니다. 삭제는 카드를 지우지 않고 요청만 기록하며, 카드 상한은 명령줄이 아니라 스킬이 적용하고, 요약 파일은 실행 전체가 아니라 아티팩트 하나의 결과를 담습니다.
- wiki 인덱스 소스가 현재 아무것도 수집하지 못한다는 사실을 문서화했습니다. deep-wiki는 envelope을 `index`로 표기하는데 이 플러그인은 `wiki-index`를 기대하므로 실제 wiki 산출물이 모두 skip되며, 그 관문을 통과하더라도 찾는 필드가 페이지에 없어 복구에는 한 줄 수정이 아니라 설계 결정이 필요합니다. 이 소스의 테스트 픽스처는 deep-wiki의 실제 출력이 아니라 기대값에 맞춰 만들어져 있어, wiki 테스트가 통과한다고 해서 통합이 동작하는 것은 아닙니다.
