...
│  - lock.js     (Node mkdir-based + stale detect, D20)           │
└────────────────────────────────────────────────────────────────┘
                              ↓
┌────────────────────────────────────────────────────────────────┐
...
│   │   ├── lock.js                 # Node mkdir-based + metadata + stale detect (D18, D20 — bash lock.sh 폐기)
...
```json
{
  ...
  "envelope": {
    ...
    "provenance": {
      "source_artifacts": [
        {
          "id": "src_0",                          // R3 — applicability[].source_id 의 reference target
...
| `applicability`, `non_applicability`, `evidence_summary`, `recommended_action`, `search_keywords`, `tags` | arrays | R3 — **min 0 허용** (Step B fallback 시 빈 배열로 persist 가능, schema validator는 통과) |
| `source_artifacts[].id` | string | R3 — `src_<index>` 형식, applicability per-source 의 `source_id` reference target |
| `source_id` (applicability/non_applicability 내) | string | R3 — `provenance.source_artifacts[].id` 와 매칭되어야 함 (validator 가 cross-reference 확인) |
...
> **v0.1.2 (2026-05-21) realignment** — v0.1.0 의 이 표는 ideal-shape 으로 작성되어
> sibling 의 실제 payload 와 4/5 개 mapper 가 mismatch (silent 0-card harvest).
> 아래는 sibling 실제 emit shape 기준으로 재작성한 표.  
> 검증 fixture: `tests/sibling-shape-smoke.test.js` (sibling repo 의 실제 fixture
> 를 직접 읽어 ≥1 card 보장).

| Source artifact | Memory type | 실제 sibling payload | 매핑 규칙 (claim source는 굵게) |
|---|---|---|---|
| `.deep-review/recurring-findings.json` | `failure-case` | `findings[].{category, severity, occurrences, example_files[], description, source_reports[]}` | **`finding.description → claim`**, `truncate(description, 80) → title`, `finding.example_files.slice(0,5) → evidence_summary`, `'category='+finding.category → applicability seed`, `[category, severity] → tags`, `envelope.generated_at → created_at` |
| `.deep-evolve/*/evolve-insights.json` | `experiment-outcome` | `payload.insights_for_deep_work[]` + `payload.insights_for_deep_review[]` 두 array의 item은 `{pattern, evidence, source_archive_ids[], suggestion}` | 두 array union. **`insight.suggestion (or insight.pattern) → claim`**, `truncate(pattern, 80) → title`, `[evidence, ...source_archive_ids].slice(0,5) → evidence_summary`, `suggestion → recommended_action`, `['evolve', 'for-deep-work'/'for-deep-review'] → tags`, `confidence: 0.5` |
| `.deep-work/*/session-receipt.json` | `pattern` / `failure-case` (outcome 분기) | `payload.{task_description, outcome, session_id, slices: {total, completed, spike}, quality_score, started_at, finished_at}` — `slices` 는 aggregate object (per-slice 데이터는 `<sid>/receipts/SLICE-*.json` 별도) | **Option A**: 1 card per session-receipt. **`'Pattern/Failure: '+task_description+' — quality '+qs+'/10 — '+completed+'/'+total+' slices completed — outcome='+outcome → claim`**, `truncate(task_description, 80) → title`, `[session_id] → evidence_summary`, `['deep-work', 'session', outcome] → tags`, `outcome ∈ {discard, abandon} → failure-case else pattern`, `quality_score/10 (clamped) → confidence` |
| `.deep-docs/last-scan.json` | `coding-style` | `payload.documents[].issues[]` (nested) — issue: `{type, category, severity, line, current_value, suggested_value, evidence}` | flatten — document 별 issues[] 를 펼쳐 1 issue = 1 card. **`'<type> at <path>:<line> — <evidence>' → claim`**, `'<type> in <path>' → title`, `['<path>:<line>'] → evidence_summary`, `'category=<category>' → applicability seed`, `suggested_value → recommended_action`, `[<category>, <severity>] → tags`, severity 매핑 → confidence (high=0.7, medium=0.5, low=0.3) |
| `<wiki_root>/.wiki-meta/index.json` | `architecture-decision` | `payload.pages[].{title, path, frontmatter}` (v0.1.0 spec과 일치 — sibling shape OK) | `page.frontmatter.adr=true` 인 페이지만. `page.title → title`, **`page.frontmatter.decision_summary || page.title → claim`**, `page.path → evidence_summary[0]`, confidence: 0.7 |
...
**Sibling shape contract**:
- 위 표는 sibling plugin 의 실제 `payload` shape 에 직접 종속.
- 각 sibling plugin 이 payload 를 변경하면 이 mapper 가 새 shape 에 맞게 업데이트되어야 함.
- 기계적 보장: `tests/sibling-shape-smoke.test.js` 가 sibling repo 의 실제 fixture 를 매 npm test 마다 harvest 해 ≥1 card 검증. sibling 이 shape 을 바꾸면 즉시 적색 신호.
...
4. **dedupe_key 충돌 검사 → 신규/병합 결정** (§6.2). **검색 범위**: `cards/<type>/<project_id>/` ∪ `cards/<type>/global/` (R3 — `--promote` 가 진행 중일 수 있으므로 local + global 양쪽 union 검색).
...
7. **FTS5 index 갱신**:
   ...
   - 순서 (R3 명확화): (a) transaction begin → (b) insert/update rows → (c) **transaction commit** → (d) sqlite handle close → (e) Step 8 에서 lock release. (b)~(d) 까지 lock 보유 — read-side §7.3.a 가 lock 으로 직렬화되므로 stale index 절대 발생 안 함.
...
**Privacy scope filter rule (R3 — 명시화)**:
- card 의 `privacy_level: local` → 동일 `project_id` 의 brief 호출에서만 후보. 다른 project 에서는 retrieval candidate set 에서 제외.
- card 의 `privacy_level: global` → 모든 project 의 brief 에서 후보. `--promote` 명시 호출로 local → global 전이된 카드만.
- project_id 가 미정 (init 미실행) → local 카드 전부 제외, global 만 retrieval. brief 헤더에 "uninitialized profile — global memories only" 경고.

| 단계 | MVP | 구현 |
|---|---|---|
| 1. Hard filter | ✅ | `status != deprecated` + **privacy scope (위 규칙)** + `review_after` age check |
...
audit:
  stale_grace_days: 90
  profile_max_age_days: 30          # R3 — project-profile.json freshness 임계값 (F19)
  high_redaction_chars: 200         # F18 — false-positive 의심 카드 임계
...
| **3b. Distill — sub-agent + bridge (Step B)** | `agents/memory-distiller.md`, `scripts/lib/llm-bridge.js` (claude-agent / codex-bash / stdin-fallback 3 adapter MVP — **stdin-fallback batch mode prototype + Q5 결정 게이트**), Step B output schema validation, golden eval harness (mock LLM), runtime-contract recorded fixtures (F12) | 2.5일 |
...
| Q5. `stdin-fallback` adapter 의 사용자 UX | (a) 매 distill 마다 prompt (시끄러움) (b) batch 모드 (배치당 1회 prompt) | 기본 (b); **결정 게이트 = Phase 3b 종료 시점** (prototype 결과로 (a)/(b) 확정) |
