...
var require_path_utils = __commonJS({
  "scripts/lib/path-utils.js"(exports2, module2) {
    ...
    function expandHomePath(raw, { homeDir = os2.homedir(), pathApi = path3 } = {}) {
...
var require_redact = __commonJS({
  "scripts/lib/redact.js"(exports2, module2) {
    ...
    var HOME_RE = createHomeRegex(os2.homedir());
    ...
    function normalizedPath(value, platform) {
      if (platform === "win32") return path3.win32.normalize(withoutExtendedDrivePrefix(value));
    ...
    function redactObject(value, opts = {}) {
      ...
      if (typeof value === "string") return redactString2(value, opts);
...
var require_lock = __commonJS({
  "scripts/lib/lock.js"(exports2, module2) {
    "use strict";
    ...
    var BACKOFF_MS = 50;
    var MAX_RETRIES = 60;
    var StaleLockError = class extends Error {
    ...
    function inspectLock(lockPath) {
      const metaPath = path3.join(lockPath, "metadata.json");
      try {
        const dirStat = fs2.lstatSync(lockPath);
        if (dirStat.isSymbolicLink() || !dirStat.isDirectory()) return null;
        const bytes = fs2.readFileSync(metaPath);
        const metaStat = fs2.lstatSync(metaPath);
    ...
    function inspectEmptyLock(lockPath) {
      try {
        const dirStat = fs2.lstatSync(lockPath);
        if (dirStat.isSymbolicLink() || !dirStat.isDirectory()) return null;
        if (fs2.readdirSync(lockPath).length !== 0) return null;
        return {
          kind: "empty-lock",
          lockPath,
          directoryIdentity: identity(dirStat),
    ...
    function sameClaim(left, right) {
      return Boolean(left && right && left.lockPath === right.lockPath && left.ownerToken === right.ownerToken && left.metadataBytes === right.metadataBytes && sameIdentity(left.directoryIdentity, right.directoryIdentity) && sameIdentity(left.metadataIdentity, right.metadataIdentity));
    }
    function sameEmptyClaim(left, right) {
      return Boolean(left && right && left.kind === "empty-lock" && right.kind === "empty-lock" && left.lockPath === right.lockPath && left.observedMtimeMs === right.observedMtimeMs && sameIdentity(left.directoryIdentity, right.directoryIdentity));
    }
    ...
    async function acquire2(lockPath, { operation = "unknown" } = {}) {
    ...
    function restoreMismatchedClaim(retiredPath, lockPath) {
      try {
        if (!fs2.existsSync(lockPath)) fs2.renameSync(retiredPath, lockPath);
      } catch {
      }
    }
    function removeClaimedDirectory(lockPath, claim) {
      ...
      try {
        fs2.renameSync(lockPath, retiredPath);
      } catch {
        return false;
      }
      const retiredClaim = { ...claim, lockPath: retiredPath, metaPath: path3.join(retiredPath, "metadata.json") };
      ...
      try {
        const entries = fs2.readdirSync(retiredPath);
        if (entries.length !== 1 || entries[0] !== "metadata.json") {
          restoreMismatchedClaim(retiredPath, lockPath);
          return false;
        }
        fs2.unlinkSync(path3.join(retiredPath, "metadata.json"));
        fs2.rmdirSync(retiredPath);
        return true;
      } catch {
        restoreMismatchedClaim(retiredPath, lockPath);
        return false;
    ...
    function breakEmptyLock(lockPath, observedClaim) {
      if (!observedClaim || observedClaim.kind !== "empty-lock" || observedClaim.lockPath !== lockPath) return false;
...
var require_runtime_context = __commonJS({
  "scripts/lib/runtime-context.js"(exports2, module2) {
    "use strict";
    function detectHost2(env = process.env) {
    ...
    module2.exports = { detectHost: detectHost2, adapterForHost };
  }
});
...
var require_project_profile_schema = __commonJS({
...
// scripts/lib/project-profile-validator.js
var require_project_profile_validator = __commonJS({
  "scripts/lib/project-profile-validator.js"(exports2, module2) {
    "use strict";
    var Ajv2020 = require__().default;
    var addFormatsModule = require_dist();
    var addFormats = addFormatsModule.default || addFormatsModule;
    var schema = require_project_profile_schema();
    var PROJECT_PROFILE_SCHEMA = Object.freeze(schema);
    var ajv = new Ajv2020({ strict: true, allErrors: true });
    addFormats(ajv);
    var validate = ajv.compile(PROJECT_PROFILE_SCHEMA);
    ...
    module2.exports = { PROJECT_PROFILE_SCHEMA, validateProjectProfile };
  }
});
...
var require_project_resolver = __commonJS({
  "scripts/lib/project-resolver.js"(exports2, module2) {
    ...
    var fs2 = require("node:fs");
    var path3 = require("node:path");
    var { validateProjectProfile } = require_project_profile_validator();
    var PROJECT_SCOPE_OBSERVER = Symbol.for("deep-memory.project-scope-observer");
    ...
    function deriveProjectId(root) {
      return `proj_${crypto2.createHash("sha256").update(canonicalizeRoot(root)).digest("hex").slice(0, 12)}`;
    }
    function globalOnly(reason, projectRoot = null) {
      return { projectRoot, projectId: null, scope: "global", warning: reason, profile: null };
    }
    function resolveProjectScope2(workspaceRoot) {
      ...
      let projectRoot;
      try {
        projectRoot = fs2.realpathSync.native(workspaceRoot);
      } catch {
        return globalOnly("project_root_unresolved");
      }
      const profilePath = path3.join(projectRoot, ".deep-memory", "project-profile.json");
      let profileText;
      try {
        profileText = fs2.readFileSync(profilePath, "utf8");
      } catch {
        return globalOnly("project_profile_missing", projectRoot);
      }
      let profile;
      try {
        profile = JSON.parse(profileText);
      } catch {
        return globalOnly("project_profile_invalid_json", projectRoot);
      }
    ...
    module2.exports = {
      canonicalizeRoot,
      deriveProjectId,
      resolveProjectScope: resolveProjectScope2,
...
// scripts/hooks/common.mjs
var import_redact = __toESM(require_redact(), 1);
var import_lock = __toESM(require_lock(), 1);
var import_runtime_context = __toESM(require_runtime_context(), 1);
var import_project_resolver = __toESM(require_project_resolver(), 1);
var { redactString } = import_redact.default;
var { acquire, release } = import_lock.default;
var { detectHost: detectRuntimeHost } = import_runtime_context.default;
...
function readConfig() {
  const cfgPath = import_node_path.default.join(DEEP_MEMORY_ROOT, "config.yaml");
  try {
    const stat = import_node_fs.default.statSync(cfgPath);
    if (configCache && configCache.mtime === stat.mtimeMs) return configCache.value;
    const text = import_node_fs.default.readFileSync(cfgPath, "utf8");
...
function isEagerDistillEnabled() {
  const cfgPath = import_node_path.default.join(DEEP_MEMORY_ROOT, "config.yaml");
  try {
    const text = import_node_fs.default.readFileSync(cfgPath, "utf8");
...
var { resolveProjectScope } = import_project_resolver.default;
...
function normalizedToolOutput(sourceKind, hookPayload) {
  const isFailure = sourceKind === "hook-tool-failure" || hookPayload.hook_event_name === "PostToolUseFailure";
  if (!isFailure) return hookPayload.tool_response ?? hookPayload.tool_output;
  if (Object.hasOwn(hookPayload, "error")) {
    const failure = {
      error: typeof hookPayload.error === "string" ? hookPayload.error : String(hookPayload.error ?? "")
    };
...
function checkDedupeWindow(dedupeKey, projectId) {
  const indexPath = import_node_path.default.join(DEEP_MEMORY_ROOT, ".dedupe-window", `${projectId}.jsonl`);
  if (!import_node_fs.default.existsSync(indexPath)) return false;
  try {
    const lines = import_node_fs.default.readFileSync(indexPath, "utf8").trim().split("\n");
...
function recordDedupeWindow(dedupeKey, projectId) {
  const indexDir = import_node_path.default.join(DEEP_MEMORY_ROOT, ".dedupe-window");
  import_node_fs.default.mkdirSync(indexDir, { recursive: true });
  import_node_fs.default.appendFileSync(import_node_path.default.join(indexDir, `${projectId}.jsonl`), `${dedupeKey}|${Date.now()}
`);
}
async function normalizeAndAppend(sourceKind, hookPayload, hostHint) {
  if (!isCaptureEnabled()) return { skipped: true, reason: "capture-disabled" };
  const projectCwd = typeof hookPayload.cwd === "string" && hookPayload.cwd.trim() ? hookPayload.cwd : process.env.PROJECT_CWD || process.env.INIT_CWD || process.cwd();
  ...
  const dedupeKey = import_node_crypto.default.createHash("sha256").update(essence).digest("hex");
  ...
  const eventKey = import_node_crypto.default.createHash("sha256").update(essence + "|" + capturedAt).digest("hex");
  const record = {
    schema_version: "1.0",
    envelope: {
      producer: "deep-memory",
      producer_version: package_default.version,
      artifact_kind: "memory-hook-event",
      run_id: import_node_crypto.default.randomUUID(),
  ...
  const eventsFile = import_node_path.default.join(DEEP_MEMORY_ROOT, "events", `${ym}.jsonl`);
  import_node_fs.default.mkdirSync(import_node_path.default.dirname(eventsFile), { recursive: true });
  const lockDir = import_node_path.default.join(DEEP_MEMORY_ROOT, ".lock");
  let lockHandle;
  try {
  ...
  try {
    ...
    const fd = import_node_fs.default.openSync(eventsFile, "a");
    try {
      import_node_fs.default.writeSync(fd, JSON.stringify(record) + "\n");
  ...
  try {
    ...
    if (isEagerDistillEnabled()) {
      try {
        ...
        const distillScript = import_node_path2.default.join(pluginRoot, "scripts", "harvest.js");
        const sessionId = input.session_id || "unknown";
        const child = (0, import_node_child_process.spawn)("node", [distillScript, "--rebuild-from-events", "--session", sessionId], {
          detached: true,
          stdio: "ignore"
        });
        child.unref();
